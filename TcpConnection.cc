...
/**
 * 发送数据  写的快， 而内核发送数据慢， 需要把待发送数据写入缓冲区， 而且设置了水位回调
 */ 
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    //表示channel_第一次开始写数据，而且缓冲区没有待发送数据 (Channel向缓冲区写数据，给客户端应用响应)
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
    ...
    if (n > 0)
    {
        ...
        messageCallback_(shared_from_this(), &inputBuffer_, receiveTime);//shared_from_this:获取当前TcpConnection对象的一个智能指针
    }
    else if (n == 0)//客户端断开连接
    {
        handleClose();
